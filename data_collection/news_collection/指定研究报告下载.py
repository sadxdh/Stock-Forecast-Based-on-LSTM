...
def getNumPage(code):
    ...
    resp = requests.get(url, headers={'User-Agent': ua.ie}, proxies=getproxy(), verify=False)
    # print(resp.text)
    # page_count = 1
    ...
    while True:
        try:
            ...
            for tr in trs:
                ...
                while True:
                    try:
                        htmlpage = requests.get(url, headers={'User-Agent': ua.ie}, proxies=getproxy(), verify=False)
                    except ProxyError:
                        print(f'{ProxyError},IP失效，标题页面内容无法获取，正在更换IP。。。。')
                    else:
